...
public class Snake : MonoBehaviourPunCallbacks, IPunObservable
{
   ...
   public bool getIsLive()
    ...
    private void Start()
    {
        ...
        PV.RPC("ShieldOff", RpcTarget.AllBuffered);  //시작할때 쉴드 이펙트 끄기
    ...
    public void Update()
    {
        ...
        if (!PV.IsMine || PhotonNetwork.PlayerList.Length < maxPlayer || playerNum < maxPlayer)
        //if (!PV.IsMine)
        {
            return;
        }
    ...
    void OnTriggerEnter2D(Collider2D col)
    {
        ...
        Debug.Log("eatenShield: " + eatenShield + ", eatShieldItem: " + eatShieldItem);

        if (eatenShield == false) //eatenShield면 이 블럭은 아예 실행되지 않음.(debug로 확인) 근데 왜 독 효과가 적용되는거지? if문 로직문제는 아닌것같은데..
        {  
            //즉사독을 먹음
    ...
    private void eatShield(int viewid)
    {
        ...
        PhotonView view = PhotonView.Find(viewid);
        PhotonNetwork.Destroy(view);
        PV.RPC("makeShieldTrue", RpcTarget.AllBuffered);

    }

    [PunRPC]
    public void makeShieldTrue()
    {
        eatenShield = true;
        eatShieldItem = true;
    }
    [PunRPC]
    public void makeShieldFalse()
    {
        eatenShield = false;
        eatShieldItem = false;
    }
    ...
    {
        PV.RPC("ShieldOn", RpcTarget.AllBuffered);
        ...
        if (timeCheckShield < 0)
        {
            ...
            timeCheckShield = 5f;
            PV.RPC("makeShieldFalse", RpcTarget.AllBuffered);
            PV.RPC("popMessage", RpcTarget.AllBuffered, "보호막끝남!");
            PV.RPC("ShieldOff", RpcTarget.AllBuffered);
        }
    ...
    private void HandleInput()
    {
        ...
        // Debug.Log(gridMoveDirection.x = 0);
        if (Input.GetKeyDown(KeyCode.UpArrow) || upB)
        {
            resetArrow();
        ...
        if (Input.GetKeyDown(KeyCode.DownArrow)||downB )
        {
            resetArrow();
            if (gridMoveDirection.y != +1)
            {         
        ...
        if (Input.GetKeyDown(KeyCode.LeftArrow) || leftB)
        {
            resetArrow();
        ...
        if (Input.GetKeyDown(KeyCode.RightArrow) || rightB)
        {
            resetArrow();
            if (gridMoveDirection.x != -1)
            {
